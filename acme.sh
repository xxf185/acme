...
menu() {
    clear
    echo ""
    echo -e " ${YELLOW}----------Acme证书一键申请脚本----------${PLAIN}"
    echo ""
    echo -e " ${YELLOW}1. 安装 Acme.sh 域名证书申请脚本${PLAIN}"
    echo -e " ${YELLOW}2. 卸载 Acme.sh 域名证书申请脚本${PLAIN}"
    echo " -------------"
    echo -e " ${YELLOW}3. 申请单域名证书 (80端口申请)${PLAIN}"
    echo -e " ${YELLOW}4. 申请单域名证书 (CF API申请)(无需解析)(不支持freenom域名)${PLAIN}"
    echo -e " ${YELLOW}5. 申请泛域名证书 (CF API申请)(无需解析)(不支持freenom域名)${PLAIN}"
    echo " -------------"
    echo -e " ${YELLOW}6. 查看已申请的证书${PLAIN}"
    echo -e " ${YELLOW}7. 撤销并删除已申请的证书${PLAIN}"
    echo -e " ${YELLOW}8. 手动续期已申请的证书${PLAIN}"
    echo -e " ${YELLOW}9. 切换证书颁发机构${PLAIN}"
    echo " -------------"
